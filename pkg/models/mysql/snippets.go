...
func (m *SnippetModel) Get(id int) (*models.ProfData, error) {
	stmt := `SELECT * FROM prof_data
    WHERE id = ?`
	...
	err := row.Scan(&s.ID, &s.D2Number, &s.D2Profstroi, &s.D2Object, &s.D2Manager, &s.D2Kontragent, &s.D2ID, &s.D2Diler, &s.D2City, &s.D2Napr,
		&s.D2SumProjToSk, &s.D2SumSkidka, &s.D2SumProjWithSkidka, &s.D2SumConstrWithSkidka, &s.D2SumRabWithSkidka, &s.D2Status, &s.NoteOrder)
	if err != nil {
